...
if ( ! empty( $el_id ) ) {
	$wrapper_attributes['id'] = 'id="' . esc_attr( $el_id ) . '"';
}
if ( ! empty( $full_width ) ) {
...
if( has_shortcode($content, 'stack_hero') && 'stretch_row' == $full_width ){
	$content = str_replace(array('[vc_column][stack_hero', '[/stack_hero][/vc_column]'), array('[stack_hero', '[/stack_hero]'), $content);	
	$parallax_hero = true;
}

if( $parallax_hero ){
	if( isset($wrapper_attributes['id']) ){
		$content = str_replace('[stack_hero', '[stack_hero '. $wrapper_attributes['id'] .' ', $content);
	}
} else {
