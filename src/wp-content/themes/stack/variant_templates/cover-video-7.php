<!--META--><section class="vim" id="variant-cover-video-7" vbr="Cover Video 7" vbp="covers">
<section class="imagebg videobg height-80 text-center" data-overlay="4">
	<video data-src-mp4="" data-src-webm="" autoplay loop muted playsinline>
		<source src type="video/webm">
		<source src type="video/mp4">
	</video>
	<div class="background-image-holder">
		<img alt="image" src="<?php variant_page_builder_demo_img('hero-1.jpg'); ?>">
	</div>
	<div class="container pos-vertical-center">
		<div class="row">
			<div class="col-sm-8 col-md-7">
				<div class="wysiwyg">
					<h1>
						Build stylish, lean sites with Stack
					</h1>
					<p class="lead">
						Stack offers a clean and contemporary look to suit a range of purposes from corporate, tech startup, marketing site to digital storefront.
					</p>
				</div>
				<a class="btn btn--primary type--uppercase" href="#">
					<span class="btn__text">
						View The Demos
					</span>
				</a>
			</div>
		</div><!--end of row-->
	</div><!--end of container-->
</section>
</section><!--end of meta Section container-->